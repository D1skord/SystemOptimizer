...
procedure HideCB();
  begin
    ...
    Form1.TmpFilesCB.Hint := 'Это файлы, создаваемые определённой программой или операционной системой'#13'для сохранения промежуточных результатов в процессе функционирования'#13'или передачи данных в другую программу.';
    Form1.CacheCB.Hint := 'Это специально выделенное место памяти на жёстком диске,'#13'где хранятся копии предварительно посещённых страниц, картинок,'#13'и других необходимых для просмотра интернет-страниц файлов.';
    Form1.DumpCB.Hint := 'Файлы, которые, как правило, создаётся при различных сбоях'#13'в программном обеспечении (например, в операционной системе),';
    Form1.RecycleBinCB.Hint := 'Элемент графического интерфейса пользователя, предназначенный для удаления и,'#13'часто, временного хранения удалённых объектов';
    Form1.DownloadsCB.Hint := 'Эта папка установлена по умолчанию для сохранения файлов с Веб браузера.';
  end;
...
procedure SetServiceWin10();
  ...
  begin
    ...
    Svc.Superfetch(Form1.CB13);
      ...
      Form1.CB14.Destroy;
      Form1.CB15.Destroy;
      Form1.MainScrollBox.VertScrollBar.Range := 600;
end;
...
procedure TForm1.ApplyClick(Sender: TObject);
  begin
    ...
    Form1.ProgressLabel2.Caption := 'Готово!';
 end;
